...
(function() {
  ...
  function Config(){
    ...
    that.store = function(dataToStore) {
      ...
      that.on('write.data.to.temp.file.error', function(error) {console.error(error);});
      that.on('unlink.backup.config.file.error', function(error) {console.error(error);});

      that.on('did.write.data.to.temp.file', function(tempFile) {
        console.log('wrote data to ' + tempFile.path);

        backupConfigFile();
      });

      that.on('did.unlink.backup.config.file', function() {
        console.log('did.unlink.backup.config.file');

        renameConfigFileToBackup();
      });

      that.on('did.rename.config.file.to.backup', function() {
        renameTempFileToConfigFile(tempFile);
      });

      that.on('did.rename.temp.file.to.config.file', function() {
        console.log('renamed ' + tempFile.path + ' to ' + kConfigFilename);
      });
      ...
      fs.writeFile(tempFile.path, _data, 'utf-8', function(error) {
        ...
        fs.rename(kConfigFilename, kConfigBackupFilename, function(error) {
          if (error) {console.error(error); return;}
          console.log('renamed ' + kConfigFilename + ' to ' + kConfigBackupFilename);
